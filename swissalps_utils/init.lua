...
if '(0.1,0.1,0.1)' ~= minetest.pos_to_string({x = 0.1111, y = 0.1111, z = 0.111}, 1) then
	local outdatedPosToString = minetest.pos_to_string;
	function minetest.pos_to_string(tPos, iDigits)
		if nil == tPos then
			return '(nil)';
		end; -- if no pos given
	...
	end; -- minetest.pos_to_string
end; -- if still old version of minetest.pos_to_string
